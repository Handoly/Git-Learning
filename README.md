...
在`github`新建仓库，记住不要勾选`添加README.md文件`的选项

在本地新建一个文件夹

将项目文件放入该文件夹

打开Git Bash软件，在命令行输入 `cd` 命令，进入该文件夹

初始化仓库：`git init` 

添加内容到**暂存区**：`git add "文件/文件夹名"`

将暂存区内容添加到本地仓库：`git commit -m "备注信息"`

将当前分支（默认分支：master）强制重命名为`main` 分支
...
●	8.同步远程仓库
●	如果远程仓库有Readme.md文件，则将Readme.md文件从远程仓库拿到本地来
●	git pull -rabase origin main
●	查看本地是否多了一个Readme.md文件
●	9.将本地项目推送到远程仓库
●	git push -u origin main
●	10.刷新GitHub页面
```
